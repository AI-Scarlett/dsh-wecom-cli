# Third-Party Notices

## WeCom CLI Skills

- Upstream: https://github.com/WecomTeam/wecom-cli
- Baseline Commit: `1d9e74026455a85d8d10a917b4d453ed4b9a829e`
- Upstream package version: `@wecom/cli@1.1.0`
- License: MIT
- Copyright: Copyright (c) 2026 WeCom

v0.1.0 redistributed upstream Skill text, references, templates and one Python helper under the MIT License. The v0.1.2 hardening release removes those executable reference materials and rewrites the 14 Skill entrypoints as minimal instructions for a plugin-owned, read-only DSH Host Tool.

The WeCom CLI executable and platform binaries are not included. Users obtain and authorize them separately from the official distribution.
