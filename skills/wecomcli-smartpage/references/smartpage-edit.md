...
2. **下载到本地**：对每个图片 URL，仅在用户明确要求分析图片、且当前环境提供受控图片下载能力时下载到一次性临时目录；不得使用 curl、wget、Python 或任意未声明的网络工具绕过 `wecom-cli`。下载失败（403 / 网络不通 / 链接过期）时停止该图片并如实说明，不编造内容。
...
- **正文图片仅走受控下载能力 + 外部图片解析**：`markdown` 正文里的 `![](<URL>)` 是外部 CDN 直链。需要理解图片内容时，仅在用户明确要求且宿主提供受控图片下载能力时处理；不得调用 curl、wget、Python 或任意未声明网络工具，也不得把 URL 塞给 `wecom-cli media download`（它只接受 `media_id`）。纯结构、搬运或覆盖类任务无需下载图片，URL 原样保留。详见上文「正文图片解析」小节。
