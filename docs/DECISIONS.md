...
- v0.3.0 outcome: complete first use inside the settings page with separately confirmed fixed-argv CLI installation and QR authorization while retaining bounded read-only business Tools.
...
| Copy-only CLI installation guidance | No Host write | User must leave DSH and cannot complete onboarding directly | Rejected for v0.3.0 |
| Confirmed fixed-argv CLI installation | One continuous setup flow; package and version cannot be changed by Browser input | Global npm write and permission failures require explicit status/recovery | Selected for v0.3.0 |
| Operation-specific read-only Host bridge | Fixed argv, central budgets and redaction, testable fail-closed behavior | Writes and file operations temporarily unavailable | Retained for v0.3.0 |
...
| Install pinned official CLI | Host onboarding | exact confirmation only | fixed npm argv / 3 minutes / bounded output | cancel or generic recovery command |
...
| Remote write or destructive operation | none in v0.3.0 | no | operation absent | fail closed |
| Upload/download/import/export | none in v0.3.0 | no | operation absent | fail closed |
...
- Approved Store update: public immutable v0.3.0 source, Registry verification, merge and GUI readback.
