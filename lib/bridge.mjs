...
const SENSITIVE_ID_KEYS = /^(?:userid|open_vid|department_id|chat_id|mail_id|media_id|file_id|space_id|folder_id|docid|content_id|msg_id|record_id|sheet_id|page_id|cursor)$/i
...
function inspectInput(value, depth = 0, key = '') {
  ...
  if (typeof value === 'string') {
    ...
    if (SQL_LIKE.test(value) || key.toLowerCase() === 'sql') fail('SQL_BLOCKED', 'SQL is not allowed in the v0.1.1 read-only bridge')
    return
  }
...
function parseOutput(text) {
  ...
  catch { return { message: trimmed.slice(0, 2000) } }
}
...
export function createWecomReadBridge(options = {}) {
  ...
  return {
    async execute(args = {}, exec = {}) {
      ...
      const data = redact(parseOutput(result.stdout))
